...
RUN apt update && apt install -y zfsutils-linux make curl unzip cron && rm -rf /var/lib/apt/lists/*

# Remove zfsutils-linux cron as it likely exists in the host
RUN rm -f /etc/cron.d/zfsutils-linux

RUN curl --output zfs-auto-snapshot.zip https://github.com/zfsonlinux/zfs-auto-snapshot/archive/92db087bb019957166ee70c88c6c967e5fd28a70.zip
